    ...
    <section className="mt-5 border-y border-hairline py-5" aria-label="Medication instructions">
      ...
      {open && <div id="medication-upload" className="mt-4 space-y-5">
        ...
        {photos.length > 0 && <ol className="grid grid-cols-3 gap-3">
          {photos.map((photo, i) => <li key={photo.url} className="min-w-0">
            <a href={photo.url} target="_blank" rel="noreferrer" aria-label={`Enlarge photo ${i + 1}`}>
              ...
              <img src={photo.url} alt={`Instruction sheet, photo ${i + 1}`} className="aspect-[3/4] w-full rounded border border-hairline object-contain bg-paper-sunken" />
            </a>
        ...
        {entries.length > 0 && <div className="space-y-5">
          <h2 ref={reviewHeading} tabIndex={-1} className="text-[22px] font-semibold tracking-tight">Check against your sheet</h2>
          <p className="text-[15px] text-ink-muted">Correct transcription errors using the written instructions. If a dose or date is missing, ask your department. The app cannot decide it.</p>
          {entries.map((entry, index) => {
            ...
            return <article key={entry.id} className="rounded-xl border border-hairline bg-paper p-4 sm:p-5">
              <div className="mb-3 flex items-start justify-between gap-3">
                <h3 className="text-[18px] font-semibold">{index + 1}. {d.name || 'Unreadable medicine'}</h3>
                <button type="button" className="min-h-[44px] shrink-0 text-[14px] text-blue-deep underline" aria-label={`Remove ${d.name || 'entry'}`} onClick={() => { onChange(entries.filter(e => e.id !== entry.id)); setMessage('Entry removed from the plan and future downloads.') }}>Remove</button>
              </div>
              ...
              {entry.confirmed && !issues.length ? <div>
                ...
                <button type="button" className="mt-2 min-h-[44px] text-blue-deep underline" onClick={() => update({ ...entry, confirmed: false })}>Edit instructions</button>
              </div> : <div className="space-y-4">
                <Field label="Medicine name"><Input value={d.name} onChange={e => setDraft({ name: e.target.value })} /></Field>
                <Field label="Department instruction"><select className={inputStyle} value={d.action ?? ''} onChange={e => setDraft({ action: (e.target.value || null) as MedicationDraft['action'] }, true)}>
                  <option value="">Choose the instruction</option><option value="take">Take</option><option value="hold">Do not take</option>
                </select></Field>
                {d.strength && <p className="text-[14px] text-ink-muted">Medicine details on sheet: {d.strength}</p>}
                {d.action && <>
                  {d.action === 'hold' ? <Field key="hold-start" label="Stop taking from" hint="Choose the date and time your department gave you (Singapore time).">
                    ...
                    {d.start && !d.startTime && <span className="mt-2 block text-[14px] text-ink-muted">Start date on sheet: {resolveMedicationDate(d.start, procedure.date) ?? d.start}. Complete the date and time above.</span>}
                  </Field> :
                    <Field key="take-start" label="Start taking on">
                  ...
                  {d.action === 'hold' ? <div className="border-t border-hairline pt-4">
                    <Field label="Remind me each day at" hint="Choose when you want a reminder not to take this medicine.">
                      <Input type="time" value={entry.reminderTimes[0] ?? ''} onChange={e => update({ ...entry, confirmed: false, reminderTimes: [e.target.value] })} />
                    </Field>
                    <p className="mt-2 text-[14px] text-ink-muted">If your reminder time is earlier than the hold starts, the first reminder will arrive when the hold starts.</p>
                  </div> : <div className="space-y-4 border-t border-hairline pt-4">
                    <h4 className="text-[16px] font-semibold">How much to take, and when</h4>
                    <p className="text-[14px] text-ink-muted">Copy each dose from your instructions. These times also set your calendar reminders.</p>
                    {entry.reminderTimes.map((time, i) => <div key={i} className="space-y-2">
                      <div className="grid grid-cols-2 gap-3">
                        <Field label={`Dose ${i + 1}`}>
                          <Input placeholder="e.g. 1 tablet" value={entry.doseAmounts[i] ?? ''} onChange={e => update({ ...entry, confirmed: false, doseAmounts: entry.reminderTimes.map((_, j) => i === j ? e.target.value : entry.doseAmounts[j] ?? '') })} />
                        </Field>
                        <Field label={`Take dose ${i + 1} at`}>
                      ...
                      {entry.reminderTimes.length > 1 && <button type="button" className="min-h-[44px] text-[14px] text-blue-deep underline" onClick={() => update({ ...entry, confirmed: false, draft: { ...d, frequency: entry.reminderTimes.length - 1 }, reminderTimes: entry.reminderTimes.filter((_, j) => i !== j), doseAmounts: entry.doseAmounts.filter((_, j) => i !== j) })}>Remove dose {i + 1}</button>}
                    </div>)}
                    <Button variant="secondary" disabled={entry.reminderTimes.length >= 8} onClick={() => update({ ...entry, confirmed: false, draft: { ...d, frequency: entry.reminderTimes.length + 1 }, reminderTimes: [...entry.reminderTimes, ''], doseAmounts: [...entry.doseAmounts, ''] })}>Add another daily dose</Button>
                  </div>}
                  {d.timing && <p className="text-[15px] text-ink-muted">Written instructions: {d.timing}</p>}
                  <p className="text-[14px] text-ink-muted">All times are Singapore time. Reminders run through {resolveMedicationDate(d.end, procedure.date) && resolveMedicationDate(d.end, procedure.date)! < procedure.date ? resolveMedicationDate(d.end, procedure.date) : procedure.date}. Follow the department’s instructions after that; this does not set a restart date.</p>
                  <details className="border-t border-hairline pt-3">
                    <summary className="min-h-[44px] cursor-pointer text-[15px] text-blue-deep">Earlier last day on your sheet?</summary>
                    <Field label="Last day these instructions apply (if specified)" hint="Leave blank to show reminders through procedure day.">
                ...
                {issues.length > 0 && <Notice tone="alert"><p className="font-semibold">Not ready to add</p><ul className="mt-2 list-disc space-y-1 pl-4">{issues.map(issue => <li key={issue}>{issue}</li>)}</ul>
                  {d.issues.length > 0 && <p className="mt-2">Retake the photo with clearer instructions to resolve the flagged items.</p>}</Notice>}
                <Button disabled={issues.length > 0 || busy} onClick={() => { update({ ...entry, confirmed: true }); setMessage(`${d.name} added. The plan and calendar download now include these reminders.`) }}>I checked this — add to my plan</Button>
              </div>}
            </article>
          })}
