...
export function ReadinessSummary({ diet, prep, stool, phone, concerning }: {
  ...
  return (
    <Card>
      <SectionTitle>{tx("04 · Overall bowel readiness")}</SectionTitle>
      ...
      <dl className="my-5 space-y-4 border-y border-hairline py-4 text-[15px]">
        ...
        <div><dt className="font-semibold text-ink">{tx("Low-residue diet")}</dt><dd className="mt-1 text-ink-muted">{tx(LABELS.diet[diet])}</dd></div>
      </dl>
      ...
      <p className="text-[15px] leading-relaxed text-ink-muted">{tx(diet === 'good' ? 'You followed the diet — thank you for keeping track.' : diet === 'unknown' ? 'Add your diet check-ins when those days are complete.' : 'Tell your team about diet slips. Keep following the diet for your current prep stage.')}{' '}{tx("Diet alone does not change this flag.")}</p>
      {concerning ? <p className="mt-3 text-[15px] font-semibold text-ink">{tx("You reported very dark or blood-like output. Contact your care team, whatever the flag colour.")}</p> : null}
